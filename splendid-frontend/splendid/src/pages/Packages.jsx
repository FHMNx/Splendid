...
const PackageCard = ({ pkg, isSelected, onSelect }) => {
  ...
  return (
    <article
      ...
      className={`relative flex cursor-pointer flex-col rounded-2xl border-2 bg-white p-6 shadow-lg transition-all duration-300 hover:-translate-y-1 hover:shadow-xl ${
        isSelected
    ...
    >
      ...
      <div className="flex items-start justify-between">
        ...
        <div className={`h-5 w-5 rounded-full border-2 transition-all ${
          ...
          {isSelected && (
            <div className={`m-0.5 h-3 w-3 rounded-full ${
      ...
      <div className="mt-6 space-y-2">
        ...
        {/* Pay Button — PayHere placeholder */}
        <button
          type="button"
          onClick={(e) => {
            e.stopPropagation();
            alert(`PayHere payment gateway coming soon!\n\nPlan: ${pkg.name}\nAmount: ${pkg.priceLabel}\n\nFor now, please contact us via WhatsApp to complete your subscription.`);
          }}
          className={`flex w-full items-center justify-center gap-2 rounded-xl px-4 py-2.5 text-sm font-semibold transition-all duration-200 ${colors.pay}`}
        >
